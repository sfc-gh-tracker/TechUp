...
def generate_sql_with_intelligence_aisql(
    session: Session,
    question: str,
    allowed_tables: List[str],
    schema_card: str,
    ...
    payload = {
        ...
        "hints": (
            "Return only a single SELECT statement. Use fully qualified identifiers.\n"
            "Only reference tables and columns from this catalog summary:\n" + schema_card
        ),
    }
    sql = f"select snowflake.cortex.aisql(parse_json($${json.dumps(payload)}$$)) as s"
...
backend = st.radio("Generation backend", ["Snowflake Intelligence AISQL", "Cortex COMPLETE"], index=0, horizontal=True)
...
if st.button("✨ Generate SQL", type="primary"):
    if not allowed_tables:
        ...
        with st.spinner("Generating SQL..."):
            ...
            if backend == "Snowflake Intelligence AISQL":
                res = generate_sql_with_intelligence_aisql(
                    session=session,
                    question=prompt,
                    allowed_tables=allowed_tables,
                    schema_card=schema_card,
                )
                generated_sql = str(res).strip().strip('`')
            else:
                system = "\n".join(FEW_SHOTS)
                full_prompt = f"""
