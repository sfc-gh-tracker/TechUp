...
create or replace schema if not exists TECHUP.QPO;
create or replace table TECHUP.QPO.SALES as
select seq4() as id,
...
select sum(amount) from TECHUP.QPO.SALES where region in ('NORTH','SOUTH');
select avg(amount) from TECHUP.QPO.SALES where region in ('NORTH','SOUTH') and order_date >= to_date('2024-06-01');

create or replace table TECHUP.QPO.CUSTOMERS as
select seq4() as cust_id, randstr(10,random()) as name from table(generator(rowcount=>50000));

select c.name, sum(s.amount)
from TECHUP.QPO.CUSTOMERS c
join TECHUP.QPO.SALES s on s.cust_id = c.cust_id
