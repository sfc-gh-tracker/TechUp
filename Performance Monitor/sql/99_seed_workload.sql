...
create or replace schema if not exists DEMO.WORKLOAD;
create or replace table DEMO.WORKLOAD.BIG_FACT as
select
  seq4() as id,
...
select count(*) from DEMO.WORKLOAD.BIG_FACT where amount > 900;
select sum(amount) from DEMO.WORKLOAD.BIG_FACT where trx_date between to_date('2024-06-01') and to_date('2024-07-01');
select bf.customer_id, sum(amount) from DEMO.WORKLOAD.BIG_FACT bf group by 1 order by 2 desc limit 1000;
...
from DEMO.WORKLOAD.BIG_FACT a
join DEMO.WORKLOAD.BIG_FACT b
